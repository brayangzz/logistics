...
import {
  MapPin, ChevronDown, Check, Pencil, Package,
  Route, Weight, Calendar, Warehouse, Truck, X,
} from "lucide-react";
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            <div key={label} className="flex flex-col items-center justify-center gap-1.5 py-2.5 rounded-xl border"
              ...
              <span className="text-[9px] font-semibold uppercase tracking-wider" style={{ color: "var(--text-secondary)" }}>{label}</span>
              <span className="text-[13px] font-semibold leading-none" style={{ color: "var(--text-primary)" }}>{value}</span>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col gap-2.5">
          <div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${order.warehouses.length}, 1fr)` }}>
            {order.warehouses.map(w => (
              <div key={w}
                ...
                style={{ backgroundColor: "var(--bg-tertiary)", borderColor: "var(--border-color)" }}>
                <div className="w-6 h-6 rounded-md flex items-center justify-center" style={{ backgroundColor: "#155DFC" }}>
                  <Warehouse className="w-3.5 h-3.5 shrink-0" style={{ color: "#fff" }} strokeWidth={1.75} />
                </div>
                <span className="text-[11px] font-medium leading-tight" style={{ color: "var(--text-primary)" }}>{w}</span>
              </div>
            ))}
          </div>
          ...
          {order.anticipated && (
            <div className="flex flex-col gap-2 px-3 py-2.5 rounded-xl"
              style={{ backgroundColor: "var(--bg-tertiary)", border: "1px solid var(--border-color)" }}>
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-bold" style={{ color: "var(--text-primary)" }}>Preparación</span>
