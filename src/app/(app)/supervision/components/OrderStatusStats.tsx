...
export function OrderStatusStats({ stats }: { stats: Stats }) {
  const cards = [
    { id: "total", label: "TOTAL PEDIDOS", value: stats.total,     color: "#6366F1", icon: Package      },
    { id: "pend",  label: "PENDIENTES",    value: stats.pendiente, color: "#F59E0B", icon: Clock        },
    { id: "ruta",  label: "EN RUTA",       value: stats.enRuta,    color: "#155DFC", icon: Truck        },
    { id: "ent",   label: "ENTREGADOS",    value: stats.entregado, color: "#10B981", icon: CheckCircle2 },
  ];
        ...
        return (
          ...
          >
            ...
            <div className="absolute inset-0 pointer-events-none"
              style={{ background: `radial-gradient(ellipse at top right, ${s.color}0D, transparent 70%)` }} />
            <div className="flex items-start justify-between gap-2 mb-3">
              <p className="text-[9px] sm:text-[10px] font-bold uppercase tracking-widest leading-tight"
                style={{ color: "var(--text-muted)" }}>
                {s.label}
              </p>
              <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full flex items-center justify-center shrink-0 transition-colors"
                style={{ backgroundColor: `${s.color}14`, border: `1px solid ${s.color}33`, color: s.color }}>
                <Icon className="w-4 h-4 sm:w-5 sm:h-5 text-current" />
              </div>
            </div>
            <motion.p
              className="font-black tabular-nums"
              style={{ fontSize: "clamp(2.5rem, 5vw, 3.5rem)", lineHeight: "1", color: "#FFFFFF" }}
